/*  tcc.cpp - Main Code of  the Program
...
vector<string> FunctionsDirectory(string dir, string tid_id){
    ...
    if(file.is_open()){        
        while(getline(file, function)){
            ...
            try {
                if(apply_command_fixed_error(command));
                else {
                    throw (str);
                }
            } catch (string str_0){
                cout << "The path already exist: " << command << endl;
                function_aux = function_aux + "__" + to_string(fixed_error);

                str_0 = "mkdir " + dir + function_aux;
                command = str_0.c_str();

                cout << "Replaced by: " << command << endl;
                fixed_error += 1;
...
bool apply_command_fixed_error (const char *command){
    if(system(command)){        
        return false;
    } else {
        return true;

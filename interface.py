...
def first_step_algorithm (mensagem, user_choices, frame): 
	# Verification if some brand was select
	if mensagem  != "" :
		command_dir = ""
		user_choices.append(mensagem)
		for choice in user_choices :
			command_dir = command_dir + choice + "/"
		if messagebox.askyesno('pop-up information', 'You select: ' + mensagem) :
...
def third_screen (root, old_frame, user_choices):
	...
	entry.grid(row = 1, column = 0, sticky = N, pady = 2, padx = 10, ipady = 5)  
